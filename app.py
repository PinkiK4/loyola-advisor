...
def analyze_data(audit_file, transcript_file):
    ...
    # Extract QPA and Credits from the transcript final summary
    qpa_m = re.search(r"QPA:.*?(\d\.\d{3})", t_text, re.DOTALL)
    qpa = qpa_m.group(1) if qpa_m else "3.548"
    
    ce_m = re.search(r"CE:\s+(\d+\.\d+)", t_text)
    total_ce = float(ce_m.group(1)) if ce_m else 111.0
    
    # Mapping completed/transfer history
    taken = set(re.findall(r"([A-Z]{2}\s\d{3})", t_text))
    # Filter Audit for incomplete requirements, focusing on Capstones and Major Electives
    audit_matches = re.findall(r"(?:Not Started|In-Progress)\s+([A-Z]{2}\*?\d{3})\s+([A-Za-z\s&]+)", a_text)
    
    recs = []
    seen = set()
    # Priority sorting: Capstones and Major requirements first
    ...
    return {
        ...
        "qpa": qpa, "total": total_ce, "recs": final_recs
    }
...
if a_f and t_f:
    ...
    st.warning("Please upload both Degree Audit and Transcript to generate analysis.")
